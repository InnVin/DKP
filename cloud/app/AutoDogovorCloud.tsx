...
export default function AutoDogovorCloud() {
  ...
  return <main className="app-shell">
    ...
    {!archive && step === 2 && <section className="workspace review"><Title title="Готовые файлы" text="XLS заполняется по BAZA.xls; PDF — одна страница A4; JPEG — 2480×3508."/><div className="preview-paper"><h2>ДОГОВОР КУПЛИ-ПРОДАЖИ АВТОМОБИЛЯ</h2><p>{val(deal, "seller_full_name")} → {val(deal, "buyer_full_name")}</p><p>{val(deal, "vehicle_make_model")} · {val(deal, "vehicle_vin")}</p></div><div className="ready-actions"><button onClick={() => downloadUrl(`/api/deals/${deal.id}/export`)}><FileSpreadsheet/> XLS</button><button onClick={() => void exportImage("pdf")}><FileText/> PDF</button><button onClick={() => void exportImage("jpeg")}><FileImage/> JPEG</button></div></section>}
    {!archive && <button className="next-button" onClick={() => setStep(current => Math.min(2, current + 1))}>Далее <ChevronRight/></button>}
    {archive && <section className="workspace"><Title title="Архив ДКП" text="Все действующие договоры и фотографии хранятся отдельно."><button className="secondary" onClick={() => downloadUrl("/api/deals/export")}><Download/> Вся база XLS</button></Title><div className="archive-grid">{deals.map(item => <article className="card archive-card" key={item.id}><div><b>ДКП № {val(item, "contract_number") || "без номера"}</b><small>{formatDate(item.contract_date)} · {val(item, "seller_full_name") || "продавец не указан"}</small><small>{val(item, "vehicle_make_model") || "автомобиль не указан"} · {val(item, "vehicle_plate") || val(item, "vehicle_vin")}</small></div><footer><button className="danger" onClick={() => removeDeal(item)}><Trash2/> Удалить</button><button className="primary" onClick={() => void openDeal(item.id)}>Открыть <ChevronRight/></button></footer></article>)}</div></section>}
