from __future__ import annotations

import base64
import json
import mimetypes
import urllib.error
import urllib.parse
import urllib.request
from pathlib import Path
from typing import Any

from .ai_ocr import FIELD_NAMES, SYSTEM_PROMPT
from .env_config import get_config

MODEL = "gemini-2.5-flash"
API_URL = "https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent"
KEY_FILE = Path(__file__).resolve().parents[1] / "config" / "gemini_api_key.txt"
OCR_EXTENSIONS = {".jpg", ".jpeg", ".png", ".webp", ".heic", ".bmp", ".tif", ".tiff", ".pdf"}


def api_key() -> str:
    key = get_config("GEMINI_API_KEY").strip()
    if key:
        return key
    if KEY_FILE.exists():
        return KEY_FILE.read_text(encoding="utf-8").strip()
    return ""


def status() -> dict[str, Any]:
    return {
        "configured": bool(api_key()),
        "model": MODEL,
        "key_file": str(KEY_FILE),
    }


def can_process(path: Path) -> bool:
    return path.suffix.lower() in OCR_EXTENSIONS


def _file_part(path: Path) -> dict[str, Any]:
    mime, _ = mimetypes.guess_type(path.name)
    if path.suffix.lower() == ".heic":
        mime = "image/heic"
    if not mime:
        mime = "application/pdf" if path.suffix.lower() == ".pdf" else "application/octet-stream"
    return {
        "inlineData": {
            "mimeType": mime,
            "data": base64.b64encode(path.read_bytes()).decode("ascii"),
        }
    }


def _extract_json(text: str) -> dict[str, Any]:
    text = text.strip()
    if text.startswith("```"):
        text = text.strip("`")
        if text.lower().startswith("json"):
            text = text[4:].strip()
    start = text.find("{")
    end = text.rfind("}")
    if start >= 0 and end > start:
        text = text[start : end + 1]
    return json.loads(text)


def _hybrid_rules() -> str:
    return """
Критически важные правила извлечения:
- Для КАЖДОГО поля сначала независимо найди все варианты значения на всех страницах комплекта, затем сопоставь их посимвольно и выбери итог. Не бери первое найденное значение.
- Если одно значение подтверждено несколькими документами, используй подтверждённый вариант. Если поле присутствует только в одном документе, используй его лишь при хорошей читаемости.
- При конфликте учитывай приоритет источников: актуальный паспорт важнее старого ДКП для данных человека; ПТС/СТС/СОР важнее старого ДКП для автомобиля; более чёткий печатный оригинал важнее рукописного текста и сырого OCR.
- Если конфликт нельзя надёжно разрешить по изображениям, не возвращай сомнительное поле и опиши расхождение в warnings. Не исправляй значения по жёстким правилам для конкретных людей, автомобилей или номеров.
- Паспорт РФ: ФИО собирай из строк рядом с метками Фамилия, Имя, Отчество. Не включай ЛИЧ, Пол, МУЖ, ЖЕН, 866, подпись.
- Дата рождения берётся только из блока разворота паспорта рядом с Дата рождения/Место рождения. Не используй дату выдачи как дату рождения.
- Дата выдачи берётся только из блока Паспорт выдан/Дата выдачи.
- Кем выдан: только название органа выдачи. Не включай 866, код подразделения, Дата выдачи, дату, Личный код, подпись.
- Если видна только прописка/регистрация без лицевой части паспорта с ФИО, номером паспорта и датой выдачи, не заполняй поле Кем выдан.
- Адрес берётся только со страницы регистрации/места жительства. Не включай даты регистрации/снятия, УФМС/ОВМ, Код подразделения, Заверил.
- Если страницы прописки нет или адрес регистрации не виден, не заполняй seller_address/buyer_address. Место рождения не является адресом.
- Для seller_passport заполняй только поля seller_*. Для buyer_passport заполняй только поля buyer_*.
- Если пользователь загрузил комплект ПТС/СТС вместе, сверяй оба документа и выбирай значение с лучшей читаемостью/уверенностью, печатный текст важнее рукописного.
- Номер кузова сверяй посимвольно по КАЖДОМУ изображению комплекта. Сначала независимо прочитай значение на каждой странице, затем сравни кандидаты. Выбирай только значение, подтверждённое минимум двумя документами, либо значение с единственного отчётливо читаемого оригинала при полной нечитабельности остальных. Не исправляй символы по заранее заданным моделям кузова и не выбирай первый найденный вариант. При конфликте похожих символов S/8, O/0, I/1 без достаточного подтверждения не возвращай поле body_number и добавь конфликт в warnings.
- СТС/СОР: это номер свидетельства на розовой бумаге, обычно внизу или сбоку. Не бери госномер автомобиля в поле СТС. Допустимые форматы только "0000 000000" или "00ХХ 000000".
- Госномер записывай кириллицей: буква + 3 цифры + 2 буквы + регион 1-3 цифры. Латинские похожие буквы A/B/E/K/M/H/O/P/C/T/Y/X переводи в кириллицу.
- ПТС/ЭПТС: используй все фото комплекта. Если VIN не ровно 17 латинских символов без I/O/Q, запиши значение в body_number, а vin не заполняй.
- Номер ПТС: электронный ПТС - 15 цифр слитно "000000000000000"; бумажный ПТС - "00ХХ 000000" или "0000 0000".
- vehicle_type означает именно категорию ТС. Ищи графу "Категория ТС" или "Категория" в ПТС, СТС или СОР и возвращай значение точно как в документе, например "B", "B/M1", "C".
- Марка/модель автомобиля должна быть фактическим названием, а не словом ТС, Марка, Модель или названием поля.
- Если поле спорное, не включай его или ставь confidence ниже 0.7.
""".strip()


def recognize_files(paths: list[Path], document_hint: str = "auto", ocr_text: str = "") -> dict[str, Any]:
    key = api_key()
    if not key:
        raise RuntimeError("Gemini API key не настроен")

    ocr_paths = [path for path in paths if can_process(path)]
    if not ocr_paths:
        return {"fields": {}, "field_meta": {}, "warnings": ["Нет файлов для OCR"]}

    prompt = (
        f"{SYSTEM_PROMPT}\n\n"
        f"{_hybrid_rules()}\n\n"
        f"Тип комплекта, выбранный пользователем: {document_hint}.\n"
        f"Разрешённые поля: {', '.join(FIELD_NAMES)}.\n"
        "Проанализируй все вложенные изображения/PDF как один комплект документов. "
        "Номера страниц соответствуют порядку вложенных файлов.\n"
        "Сырой текст Yandex OCR ниже может содержать ошибки и нужен как подсказка, изображения важнее:\n"
        f"--- OCR TEXT START ---\n{ocr_text[:12000]}\n--- OCR TEXT END ---\n"
        "Верни строго JSON без markdown: "
        '{"document_type":"...",'
        '"fields":{"field_name":{"value":"...","confidence":0.0,"page":1,"evidence":"..."}},'
        '"warnings":["..."]}'
    )
    payload = {
        "contents": [
            {
                "role": "user",
                "parts": [{"text": prompt}, *[_file_part(path) for path in ocr_paths]],
            }
        ],
        "generationConfig": {
            "temperature": 0,
            "responseMimeType": "application/json",
        },
    }
    url = API_URL.format(model=urllib.parse.quote(MODEL, safe="")) + "?key=" + urllib.parse.quote(key)
    request = urllib.request.Request(
        url,
        data=json.dumps(payload, ensure_ascii=False).encode("utf-8"),
        headers={"Content-Type": "application/json"},
        method="POST",
    )
    try:
        with urllib.request.urlopen(request, timeout=180) as response:
            response_payload = json.loads(response.read().decode("utf-8"))
    except urllib.error.HTTPError as exc:
        detail = exc.read().decode("utf-8", errors="replace")
        raise RuntimeError(f"Gemini OCR вернул ошибку: {detail[:500]}") from exc
    except (OSError, urllib.error.URLError, TimeoutError) as exc:
        raise RuntimeError(f"Gemini OCR недоступен: {exc}") from exc

    parts = response_payload.get("candidates", [{}])[0].get("content", {}).get("parts", [])
    content = "".join(str(part.get("text", "")) for part in parts).strip()
    parsed = _extract_json(content or "{}")

    fields: dict[str, str] = {}
    field_meta: dict[str, dict[str, Any]] = {}
    for key_name, item in parsed.get("fields", {}).items():
        if key_name not in FIELD_NAMES or not isinstance(item, dict):
            continue
        value = str(item.get("value", "")).strip()
        if not value:
            continue
        confidence = max(0.0, min(1.0, float(item.get("confidence", 0))))
        fields[key_name] = value
        field_meta[key_name] = {
            "confidence": confidence,
            "page": item.get("page"),
            "evidence": str(item.get("evidence", ""))[:200],
            "source": "gemini",
        }

    return {
        "document_type": parsed.get("document_type", document_hint),
        "fields": fields,
        "field_meta": field_meta,
        "warnings": parsed.get("warnings", []),
    }

